...
public class PileController : MonoBehaviour
{
    ...
    void Update()
    {
        // Vérifie si le bouton de la souris droit est enfoncé
    ...
    public void ReadDataCSV()
    {
        ...
        // Parcourez chaque ligne du fichier CSV (en sautant la première ligne si elle contient des en-têtes)
        for (int i = 1; i < lignes.Length; i++)
        {
            string ligne = lignes[i];
            string[] colonnes = ligne.Split(';');

            // Vérifiez si le nombre de colonnes est correct (vous pouvez ajuster cela en fonction de la structure de votre CSV)
            ...
            // Convertissez les données en types appropriés
            int id;
            float price;
            if (!int.TryParse(colonnes[0], out id) || !float.TryParse(colonnes[1], out price))
            {
                Debug.LogWarning("Impossible de convertir les données sur la ligne " + (i + 1) + " du fichier CSV.");
                continue; // Passez à la prochaine ligne
            }
    ...
    private List<Card> Draw(int quantity)
    {
        ...
        {
            // Vérifie si la pile de cartes est vide
            ...
            // Génère un index aléatoire dans la plage des indices de la liste
            int randomIndex = Random.Range(0, CardsPile.Count);

            // Ajoute la carte piochée à la liste de cartes tirées
            cartesTirees.Add(CardsPile[randomIndex]);

            // Retire la carte piochée de la pile de cartes
