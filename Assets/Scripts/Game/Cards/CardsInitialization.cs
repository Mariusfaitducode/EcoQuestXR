...
{ 
    ...
    public static void MatchCardWithObjectProperties(List<Card> cards, List<ObjectProperties> objectsProperties, float factorCostReductionDestruction)
    {
        ...
        foreach (Card card in cards)
        {
            ...
            {
                card.cardProperties.price = (int)(card.objectProperties1.price * card.quantityObject1 * factorCostReductionDestruction) + card.objectProperties2.price * card.quantityObject2;
                ...
                card.cardProperties.population = - card.objectProperties1.population * card.quantityObject1 + card.objectProperties2.population * card.quantityObject2;
                card.cardProperties.energy = - card.objectProperties1.energy * card.quantityObject1 + card.objectProperties2.energy * card.quantityObject2;
                card.cardProperties.pollution = - card.objectProperties1.pollution * card.quantityObject1 + card.objectProperties2.pollution * card.quantityObject2;
            }
            else
            {
